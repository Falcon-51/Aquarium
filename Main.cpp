...
int main()
{
    ...
    t4.loadFromFile("images/angry111.png");
    ...
    //Генерируем врагов и игрока
    list<Entity*> entities;

    for (int i = 0; i < 3; i++)
    {
        Enemy* A = new Enemy();
        A->settings(sAngry1, rand() % W, rand() % H, 0, 30);
    ...
    {
        Enemy* A = new Enemy();
        A->settings(sAngry2, rand() % W, rand() % H, 0, 30);
        entities.push_back(A);
    }
    ...
    {
        Enemy* A = new Enemy();
        A->settings(sAngry3, rand() % W, rand() % H, 0, 30);
        entities.push_back(A);
    }
    ...
    //Главный цикл
    while (app.isOpen()) 
    {   //Проверка положения курсора и нажатия на кнопку
        if ((IntRect(60, 200, 200, 80).contains(Mouse::getPosition(app))) && (Mouse::isButtonPressed(Mouse::Left)))
        {
            while (app.isOpen())
            {
                ...
                //Отработка клавиши поворота вправо
                ...
                //Отработка клавиши поворота влево
                ...
                for (auto a : entities)
                    for (auto b : entities)
                    {   //Проверка столкновения врага и пузырька
                        ...
                        //Проверка достижения правой стороны
                        ...
                        //Проверка достижения левой стороны
                        ...
                        //Проверка прохождения верхней и нижней границ
                        if (a->name == "Player" && a->y > H) a->y = 0;
                        if (a->name == "Player" && a->y < 0) a->y = H;
                    }

                //Условия анимации движения
